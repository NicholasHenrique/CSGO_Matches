...
try:
  ...
  if TABLE_NAME not in os.listdir('spark-warehouse/bronze.db'):
    ...
    # spark.sql("CREATE DATABASE bronze")
    df_new.write.format("delta").saveAsTable(name=f"{warehouse_location}.bornze.{TABLE_NAME}", mode="overwrite") # overwrite não está sobreescrevendo pois quando o comando é executado ele atribui um novo nome para o arquivo (?)
